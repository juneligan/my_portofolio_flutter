...
class AnalyticsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final totalAnalyticsDateState = ref.watch(
      dateRangeProvider
          .select((state) => state[DateRangeKey.totalAnalytics.getKey()]),
    );
      ...
      child: analyticsState.when(
        data: (data) => Column(
          ...
          children: [
            ...
            // GRAPH
            buildGraph(context, ref, data, true),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                const AppText(
...
SizedBox buildGraph(
  ...
  WidgetRef ref,
  List<TotalClickEventResponse> data,
                          ...
                          sideTitles: SideTitles(
                            ...
                            getTitlesWidget: (value, meta) {
                              return Transform.rotate(
                                angle: -0.5,
                                child: AppText(
                                  _formatDate(data[value.toInt()].clickDate),
                                  type: TextType.xs,
                                ),
                              );
                            },
          ...
          Column(
            ...
            children: [
              ...
              const Text(
                ...
                style: TextStyle(fontSize: AppSizes.txtSm, color: Colors.grey),
              ),
              AppSizedBox.lg(),
              if (showCreateButton) _createShortUrlButton(context, ref),
            ],
          ),
      ],
    ),
...
AppButton _createShortUrlButton(BuildContext context, WidgetRef ref) {
  return AppButton(
...
void _copyToClipboardWeb(String text) {
    ...
    children: [
      AppText(
        dateState?.startDate != null
            ? dateState!.startDate!.toIso8601String()
            : 'start date',
        type: TextType.md,
      ),
      ...
      AppText(
        dateState?.endDate != null
            ? dateState!.endDate!.toIso8601String()
            : 'end date',
        type: TextType.md,
      ),
      AppSizedBox.xl(horizontal: true),
      AppDateRangeSelector(
        keyId: DateRangeKey.totalAnalytics.getKey(),
      ),
      AppSizedBox.xl(
        horizontal: true,
      ),
      if (data.isNotEmpty && showCreateButton) _createShortUrlButton(context, ref),
      if (data.isEmpty && showCreateButton) AppSizedBox.xl(horizontal: true),
      AppSizedBox.xl(horizontal: true),
      AppButton('Reload', onPressed: () {
        notifier.fetchAnalytics(dateRange: dateState);
      })
    ],
  );
}
